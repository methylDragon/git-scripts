...
| **`git gk-optimize <install/verify/uninstall> [repo] [--config] [--keep-recent-tags]`** | **GitKraken Worktree Optimizer.** Improves GitKraken performance on large repositories and linked worktrees by pruning file-watcher directory traversal, disabling commit detail panel transition lag in memory, retaining a rolling window of CI tags, and auto-refreshing linked worktrees. See [`src/git_scripts/gk/optimize/README.md`](src/git_scripts/gk/optimize/README.md). |
...
If you use **GitKraken Desktop** on Linux with linked worktrees, run `git gk-optimize install <repo>` (see [`src/git_scripts/gk/optimize/README.md`](src/git_scripts/gk/optimize/README.md)) to prevent GitKraken's file watcher from following child symlinks or indexing generated directories, eliminate commit detail panel slide-in lag, maintain a bounded rolling window for CI tags, and automatically refresh the commit graph after terminal Git commands.
