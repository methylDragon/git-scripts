...
| **`git gk-optimize <install/verify/uninstall> [repo] [--config] [--keep-recent-tags]`** | **GitKraken Worktree Optimizer.** Prevents file-watcher symlink/build-dir stalls (`libgk_nsfw_nofollow.so`), maintains rolling top-`N` CI tag windows (`config.yaml`), and auto-refreshes linked worktrees. See [`src/git_scripts/gk/optimize/README.md`](src/git_scripts/gk/optimize/README.md). |
...
If you use **GitKraken Desktop** on Linux with linked worktrees, run `git gk-optimize install <repo>` (see [`src/git_scripts/gk/optimize/README.md`](src/git_scripts/gk/optimize/README.md)) to prevent GitKraken's file watcher from crawling build symlinks and generated directories, maintain a configurable rolling window for high-volume CI tags, and automatically refresh GitKraken's commit graph after terminal commands.
